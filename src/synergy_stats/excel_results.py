...
RESULT_SHEET_CONFIGS = (
    ResultSheetConfig(
        ...
        description="Run 수준에서 step/nonstep 그룹별 핵심 결과를 한 줄씩 요약합니다.",
        key_columns="group_id, n_trials, n_clusters, selection_status",
        column_help=(
            "group_id: global_step / global_nonstep 그룹을 구분합니다.",
            "n_trials: 해당 그룹에 포함된 trial 수입니다.",
            "n_clusters: 최종 대표 synergy cluster 개수입니다.",
            "selection_status: K 선택이 어떤 경로로 확정되었는지 보여줍니다.",
        ),
        example_lines=(
            "예를 들어 group_id=global_step 이고 selection_status=success_gap_unique 이면, step 그룹은 gap statistic이 고른 K를 그대로 최종 채택했다는 뜻입니다.",
    ...
    ResultSheetConfig(
        ...
        example_lines=(
            ...
            "같은 cluster_id 를 group_id 별로 비교하면 step/nonstep 간 대표 근육 패턴 차이를 볼 수 있습니다.",
        ),
    ...
    ResultSheetConfig(
        ...
        example_lines=(
            ...
            "group_id 별로 같은 cluster_id 를 비교하면 시간 패턴 차이를 해석할 수 있습니다.",
        ),
