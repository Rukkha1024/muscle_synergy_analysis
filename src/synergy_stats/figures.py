...
def save_group_cluster_figure(
    ...
    if has_coverage:
        ...
        suptitle = f"{_group_title(group_id)}  (n={total_trials} trials, {total_subjects} subjects)"
    else:
        coverage = pd.DataFrame()
        suptitle = _group_title(group_id)
    ...
    for row_index, cluster_id in enumerate(cluster_ids or [0]):
        ...
        if cluster_w.empty or cluster_h.empty:
            ...
            ax_w.set_title(f"Cluster {cluster_id}: W{subtitle}", fontsize=11)
            ...
            ax_h.set_title(f"Cluster {cluster_id}: H (100-window){subtitle}", fontsize=11)
