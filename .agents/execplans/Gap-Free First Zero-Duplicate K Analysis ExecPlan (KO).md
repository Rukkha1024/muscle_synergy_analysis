...
가장 쉬운 확인 방법은 `analysis/` 아래 스크립트 하나를 실행해 `outputs/final_concatenated.parquet` 같은 final parquet를 넘기고, 스크립트가 K scan 요약을 stdout에 출력하며 자기 analysis 폴더 아래에 pipeline-like artifact 묶음을 쓰고, gap-statistic 경로를 호출하지 않았다고 명시하는지 확인하는 것이다.
...
- [x] (2026-03-19 04:27Z) analysis rerun 결과를 `final.parquet`, `final_concatenated.parquet`, `concatenated/*.xlsx`, `concatenated/figures/*.png` 형태로 analysis 작업폴더에 저장하도록 확장했다.
- [x] (2026-03-19 04:34Z) `analysis_methods_manifest.json`도 output-dir 상대경로로 정규화해 `default_run`과 `recheck_run` 사이 MD5가 일치하도록 맞췄다.
- [x] (2026-03-19 04:35Z) `default_run`과 `recheck_run` 사이에서 `summary.json`, `k_scan.json`, `k_duplicate_burden.png`, `final.parquet`, `final_concatenated.parquet`, `analysis_methods_manifest.json`, mode figure PNG들의 MD5가 일치함을 확인했다.
...
- Observation: `analysis_methods_manifest.json`도 처음에는 output directory 이름 때문에 run 간 MD5가 달랐다.
  Evidence: `default_run`과 `recheck_run` diff를 보면 manifest 안의 parquet/workbook path만 달랐고, analysis 작업폴더 기준 상대경로로 재기록하자 MD5가 일치했다.

- Observation: `.xlsx` workbook은 내용이 같아도 byte-level MD5는 재현되지 않았다.
  Evidence: `clustering_audit.xlsx`와 `results_interpretation.xlsx`의 zip 내부 `docProps/core.xml`에는 openpyxl 생성 시각이 run마다 새로 기록됐다.
...
- Decision: analysis 결과는 JSON과 진단 figure만 남기지 않고, pipeline과 같은 소비 형태의 parquet/workbook/figure 묶음으로 analysis 작업폴더에 저장한다.
  Rationale: 사용자가 최종적으로 원하는 산출물은 "`K=13`이 나왔다면 그 결과를 pipeline output과 동일하게 작업폴더에 저장"하는 것이었다.
  Date/Author: 2026-03-19 / Codex

## Outcomes & Retrospective

이 계획은 이제 요청된 analysis-only 범위에서 구현이 끝났다. 저장소에는 실행 가능한 스크립트, README, report, 재현 가능한 artifact를 포함한 `analysis/first_zero_duplicate_k_rerun/` 폴더가 생겼다. `outputs/final_concatenated.parquet`에 대해 실행했을 때 analysis는 `k_selected_first_zero_duplicate=13`을 보고하고, 같은 bundle의 pipeline metadata는 여전히 gap 기반 값 `15`를 보고한다. 또한 그 no-gap 결과는 `analysis/first_zero_duplicate_k_rerun/artifacts/default_run/` 아래에 `final.parquet`, `final_concatenated.parquet`, workbook, figure 형태로 저장된다. 즉, `main.py`나 production pipeline을 바꾸지 않고도 사용자의 현재 `13 vs 15` 질문에 직접 답하고, 그 답을 pipeline과 같은 형식으로 바로 검토할 수 있게 됐다.
...
출력은 analysis 범위 안에만 머물러야 한다. artifact는 `analysis/first_zero_duplicate_k_rerun/artifacts/<run_name>/` 아래에 쓴다. artifact set은 analysis 전용 JSON에 더해 pipeline-like export를 포함한다. 즉 `summary.json`, `k_scan.json`, `checksums.md5`, `k_duplicate_burden.png` 외에도 `final.parquet`, `final_concatenated.parquet`, `concatenated/clustering_audit.xlsx`, `concatenated/results_interpretation.xlsx`, `concatenated/figures/*.png`를 함께 쓴다. `report.md`는 연구 질문, offline reconstruction 방법, 정확한 no-gap selection rule, 관찰된 K scan, 현재 bundle에 대한 최종 답을 설명해야 한다.
