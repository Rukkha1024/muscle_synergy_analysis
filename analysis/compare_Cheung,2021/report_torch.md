...
| Gap-statistic runtime | 500 reference sets and 100 restarts per reference | Script defaults are 500 reference sets and 100 restarts per reference; this run used 500 and 100 | The checked-in script defaults are paper-aligned, but local validation may still use explicit CLI overrides for tractable runtime. |
...
The plain-k-means gap-statistic search produced `step` common clusters `[0, 2, 3, 4, 5, 8, 9, 10, 11, 12]` and `nonstep` common clusters `[1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12, 13]`. Step-to-nonstep centroid matching found `8` valid pair(s), with unmatched step centroids `[0, 1]` and unmatched nonstep centroids `[0, 2, 5, 8]`.
...
| --- | --- | --- | --- | --- | --- |
| step→nonstep | 3816 | 0.331 | 0.427 | 0.352 | -0.021 |
| nonstep→step | 3816 | 0.300 | 0.336 | 0.313 | -0.013 |

At the centroid level, merging or fractionation detection returned:

- `step <- nonstep` centroid merging detections: `2/10`
- `nonstep <- step` centroid merging detections: `2/12`
...
| --- | --- | --- | --- | --- |
| Step | 7 | 0.966 | P0↔B4 (0.959), P3↔B1 (0.963), P4↔B2 (0.985), P5↔B3 (0.923), P10↔B5 (0.955), P11↔B0 (0.985), P12↔B6 (0.989) | 2, 8, 9 |
| Nonstep | 5 | 0.925 | P5↔B0 (0.980), P9↔B2 (0.935), P11↔B1 (0.929), P12↔B3 (0.809), P13↔B4 (0.974) | 1, 2, 3, 6, 7, 8, 10 |
...
| Common-cluster rule | >=1/3 subject contribution | Step common=10, Nonstep common=12 | Consistent |
| Centroid matching | Use scalar product with SP<0.8 unmatched | Matched=8, unmatched step=2, unmatched nonstep=4 | Consistent |
...
```bash
conda run --no-capture-output -n cuda python analysis/compare_Cheung,2021/analyze_compare_cheung_synergy_analysis_torch.py --dry-run
```

Run the full analysis with the paper-aligned defaults:

```bash
conda run --no-capture-output -n cuda python analysis/compare_Cheung,2021/analyze_compare_cheung_synergy_analysis_torch.py
```
...
| File | Description |
| --- | --- |
| `analysis/compare_Cheung,2021/figures_torch/figure01_rank_sparseness.png` | Paper-style rank distribution and vector sparseness by step class. |
| `analysis/compare_Cheung,2021/figures_torch/global_step_clusters.png` | Paper step common clusters rendered with the same two-column W/H layout as the pipeline. |
| `analysis/compare_Cheung,2021/figures_torch/global_nonstep_clusters.png` | Paper nonstep common clusters rendered with the same two-column W/H layout as the pipeline. |
| `analysis/compare_Cheung,2021/figures_torch/figure03_step_nonstep_matching.png` | Scalar-product heatmap between step and nonstep common centroids. |
| `analysis/compare_Cheung,2021/figures_torch/figure04_baseline_correspondence_step.png` | Paper step common centroids matched against baseline step representatives. |
| `analysis/compare_Cheung,2021/figures_torch/figure04_baseline_correspondence_nonstep.png` | Paper nonstep common centroids matched against baseline nonstep representatives. |
| `analysis/compare_Cheung,2021/figures_torch/figure05_crossfit_mi.png` | Cross-fit generalizability and individual-level MI summary. |
