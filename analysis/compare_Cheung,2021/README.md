...
이 폴더는 **Cheung et al. (2020) 논문의 muscle synergy 분석 로직**을 현재 프로젝트의 perturbation EMG 데이터셋과 `step vs nonstep` 비교 구조에 맞춰 다시 실행하고, 그 결과를 현재 main pipeline 기준 baseline bundle과 비교하기 위한 분석 작업 공간이다.

핵심은 “논문 로직을 그대로 복제”하는 것이 아니라, **현재 프로젝트의 trial 선택과 analysis window는 유지한 채** 논문식 NMF rank 선택, plain k-means + gap statistic, centroid matching, sparseness, merging/fractionation 해석을 별도 `analysis/` 공간에서 재적용하는 데 있다.

현재 main pipeline의 source of truth는 mode별 `pooled_step_nonstep` clustering이다. 다만 이 compare_Cheung 스크립트는 baseline trial window는 현행 main pipeline metadata를 따르면서도, centroid correspondence 비교를 위해 legacy `global_step` / `global_nonstep` representative export가 함께 들어 있는 **호환 baseline bundle**을 추가로 요구한다.
...
1. **NMF 기반 synergy 추출**
   - 각 subject/group의 EMG를 Non-negative Matrix Factorization(NMF)으로 분해한다.
   - 시간에 따라 변하는 activation coefficient와 시간 불변 synergy vector의 곱으로 EMG를 재구성한다.
...
2. **시너지 수 선택 규칙은 `R² ≈ 0.80`**
   ...
   - 논문 설명상 각 rank는 random initialization을 여러 번 시도하고, 그중 최고 `R²` 해를 사용한다.

3. **k-means는 plain pooled clustering이다**
   - 논문은 pooled synergy vector를 가지고 일반적인 k-means를 수행한다.
   - 한 번의 실행에서 trial 내부 uniqueness를 강제하지 않고, **후보 `K = 2..20`을 각각 따로 평가**한다.
...
5. **subject-invariant cluster 판정**
   - 각 그룹에서 pooled synergy vector를 clustering한 뒤, **해당 그룹 subject의 `1/3` 이상이 기여한 cluster**를 subject-invariant cluster로 본다.
...
2. **raw EMG preprocessing을 다시 하지 않는다**
   - 논문은 raw running EMG filtering, rectification, normalization을 바탕으로 분석한다.
   - 현재 저장소는 이미 정규화된 parquet와 event metadata를 입력 source로 사용하므로, 논문식 raw preprocessing을 처음부터 다시 재현하지 않는다.
   - 이번 작업은 **현재 프로젝트 입력에 대한 paper-aligned adaptation**이다.
...
5. **NMF 탐색 범위는 현재 데이터셋에 맞게 `1..16`으로 둔다**
   - 논문은 15근육 데이터이므로 `1..15`를 시험했다.
   - 이번 분석은 16채널 입력이므로 `1..16`을 시험한다.
   - 대신 rank 선택 규칙, restart 수, `R²` threshold 해석은 논문 쪽으로 맞춘다.

6. **clustering은 duplicate-free 보정 없이 plain k-means로 수행한다**
   - 이전 구현은 current-project adaptation으로 within-trial duplicate-free 재배정을 넣었지만, 이번 수정에서는 그것을 제거했다.
   - 따라서 clustering은 이제 **논문처럼 pooled vector에 대한 plain k-means + gap statistic**이다.
...
baseline user synergy analysis 결과는 예시로 `outputs/runs/default_run/` 아래에 둘 수 있다.  
...
- **논문 / compare_Cheung 로직**
  ...
  - 각 rank에서 random initialization을 `20`회 시도하고, 최고 `R²` 해를 보관한다.
  - 최종 rank는 **centered-variance 기준 `R² >= 0.80`을 처음 만족하는 최소 rank**다.
...
- **현재 baseline pipeline 로직**
  ...
  - baseline은 rank당 단일 적합 경로를 중심으로 순차 탐색한다.

정리하면:
- compare_Cheung: **`R² 0.80`, `1..16`, rank당 20회 restart, centered variance 기준**
- baseline pipeline: **`VAF 0.90`, `1..8`, pipeline-specific rank 탐색**

### 3.2 Clustering 차이

- **논문 / compare_Cheung 로직**
  - candidate `K = 2..20`을 각각 평가한다.
  - 각 candidate K마다 plain k-means를 `1000`회 반복한다.
  - reference data `500`개에 대해 각 `100`회씩 plain k-means를 돌려 gap statistic을 계산한다.
  - 최종 K는 **gap statistic으로 고른다**.

- **현재 baseline pipeline 로직**
  - 현재 main pipeline은 mode별 단일 `pooled_step_nonstep` group을 source of truth로 사용한다.
  - candidate K는 `k_lb >= subject_hmax` 제약 아래서 gap statistic과 zero-duplicate feasibility를 함께 고려한다.
  - 대표 cluster는 step/nonstep을 같은 pooled 공간에서 공유하고, strategy 분포는 후속 summary로 읽는다.

정리하면:
- compare_Cheung: **`K = 2..20` plain k-means + gap statistic**
- baseline main pipeline: **pooled clustering + gap statistic + zero-duplicate feasibility**

다만 현재 compare_Cheung 스크립트의 baseline correspondence 단계는 호환 baseline bundle 안에 있는 `global_step/representative_W_posthoc.csv`, `global_nonstep/representative_W_posthoc.csv`를 여전히 요구한다. 즉, 이 README에서 말하는 “baseline 비교”는 현행 main pipeline 설명과 별개로, 그 비교를 위해 준비된 호환 산출물 bundle까지 포함한 의미다.
...
- **현재 baseline pipeline 로직**
  ...
  - baseline main pipeline은 pooled grouping과 strategy summary 중심의 산출물을 만든다.
...
- **현재 baseline pipeline 로직**
  - trial window 확정
  - trial-level NMF
  - pooled step/nonstep clustering
...
`--run-dir`는 아래 파일을 포함한 **호환 baseline bundle**을 가리켜야 한다.
...
- 수정 ExecPlan: `analysis/compare_Cheung,2021/exceplan_compare_cheung_pdf_alignment_revision.md`

이 README는 **비교 기준과 방법론 차이**를 설명하는 문서이고, 실제 결과 수치와 figure 해석은 `report.md`에서 확인하면 된다. 시각화 파일 목록도 `report.md`의 Figures 섹션을 참고하면 된다.
