...
The script rebuilt each selected trial from the normalized EMG input, validated that the rebuilt windows matched baseline run `default_run`, and then ran a multiplicative-update NMF search over ranks `1..16` with `20` restarts per rank. The selected rank was the smallest rank whose reconstruction reached `R² >= 0.80`; if a trial never reached the threshold, the script kept the best-`R²` rank and marked the trial as a threshold miss.
...
The paper-style rank distributions differed only modestly from the baseline rank distribution. The current rank distribution was `{1: 23, 2: 67, 3: 34, 4: 1}`, while the baseline distribution was `{1: 1, 2: 6, 3: 37, 4: 50, 5: 26, 6: 4, 7: 1}`. The rank-delta summary relative to baseline was `{-4: 1, -3: 14, -2: 70, -1: 36, 0: 4}`.

The duplicate-free gap-statistic search produced `step` common clusters `[0, 2, 3, 4, 5, 6]` and `nonstep` common clusters `[0, 1, 2, 3]`. Step-to-nonstep centroid matching found `4` valid pair(s), with unmatched step centroids `[4, 5]` and unmatched nonstep centroids `[]`.
...
| --- | --- | --- | --- | --- | --- |
| step→nonstep | 3816 | 0.631 | 0.646 | 0.611 | 0.020 |
| nonstep→step | 3816 | 0.582 | 0.597 | 0.622 | -0.039 |

At the centroid level, merging or fractionation detection returned:

- `step <- nonstep` centroid merging detections: `3/6`
- `nonstep <- step` centroid merging detections: `2/4`
...
| --- | --- | --- | --- | --- |
| Step | 6 | 0.960 | P0↔B3 (0.983), P2↔B5 (0.898), P3↔B1 (0.991), P4↔B2 (0.947), P5↔B6 (0.965), P6↔B4 (0.974) | None |
| Nonstep | 4 | 0.967 | P0↔B0 (0.975), P1↔B2 (0.981), P2↔B4 (0.927), P3↔B1 (0.983) | None |

## Comparison with Prior Studies

| Comparison Item | Prior Study Result | Current Result | Verdict |
| --- | --- | --- | --- |
| Rank rule | ~6 vs. ~7 synergies at R²≈0.80 | Step median=2.0; Nonstep median=2.0 | Partially consistent |
| Common-cluster rule | >1/3 subject contribution | Step common=6, Nonstep common=4 | Consistent |
| Centroid matching | Use scalar product with SP<0.8 unmatched | Matched=4, unmatched step=2, unmatched nonstep=0 | Consistent |
| Cross-fit | Across-group fit compared with within-group benchmark | Across-group deltas: step→nonstep=0.020, nonstep→step=-0.039 | Consistent |
