...
위 설명만 보면 `analysis_window_end`가 항상 같은 종류의 고정 이벤트처럼 들릴 수 있다.

하지만 실제로는 trial 종류에 따라 만들어지는 방식이 다르다.

### 1. Step trial의 종료점

`step` trial은 실제 stepping event를 쓴다.

- 시작점: `platform_onset`
- 종료점: 실제 `step_onset`
- 저장 컬럼: `analysis_window_end`

즉, step trial에서는 `analysis_window_end`가 사실상 실제 `step_onset`을 가리킨다.

### 2. Nonstep trial의 종료점

`nonstep` trial은 실제 `step_onset`이 없기 때문에, 같은 subject-velocity 그룹에서 관측된 step trial의 평균 latency를 가져와 surrogate 종료점을 만든다.

쉽게 쓰면 아래와 같다.

- 먼저 같은 subject와 velocity 안에서 step trial들의 `step_onset - platform_onset` 값을 본다.
- 그 평균 latency를 nonstep trial의 `platform_onset`에 더한다.
- 그 결과를 `analysis_window_end`로 사용한다.

이렇게 해야 nonstep도 step과 비슷한 시간 의미를 가진 비교 구간으로 맞출 수 있다.

### 3. Donor가 없는 예외 처리

가끔은 같은 subject-velocity 그룹 안에 surrogate를 만들 step donor가 없을 수 있다.

이 경우 현재 helper는 해당 nonstep trial의 종료점을 `platform_offset`으로 둔다.

즉, 현재 전처리 원칙은 아래 순서로 이해하면 된다.

1. step이면 실제 `step_onset`까지 자른다.
2. nonstep이면 같은 그룹 step trial의 평균 latency로 surrogate end를 만든다.
3. 그마저 불가능하면 `platform_offset`을 fallback으로 쓴다.

이 부분이 중요한 이유는, CNN이 보는 비교 구간이 "step은 실제 발내딛기 전까지", "nonstep은 그에 대응하도록 맞춘 surrogate 구간까지"라는 뜻이기 때문이다.

## 무엇이 main pipeline과 같고, 무엇이 CNN에서만 추가되나?

혼동하기 쉬운 지점이라 구분해서 보는 편이 좋다.

### main pipeline과 같은 부분

아래 단계는 기존 EMG pipeline helper를 그대로 따른다.

- event workbook에서 `step`과 `nonstep` label을 준비한다.
- `mixed comparison` 기반 filtering을 적용한다.
- `analysis_selected_group`, `analysis_is_step`, `analysis_is_nonstep`를 만든다.
- 각 trial을 `platform_onset`부터 `analysis_window_end`까지 자른다.

즉, 어떤 trial이 분석에 포함되는지와 어디까지 자르는지는 main pipeline과 같은 기준을 쓴다.

### CNN에서 추가되는 부분

CNN은 trial 길이가 모두 같아야 입력으로 묶기 쉽다.

그런데 실제 trial window 길이는 trial마다 조금씩 다를 수 있다.

그래서 CNN 프로토타입은 기존 helper로 trial을 먼저 자른 뒤, 그 잘린 구간을 다시 고정 길이로 맞춘다.

현재 기준으로는 아래처럼 처리한다.

- 입력 채널: 16개 EMG 채널
- trial별 원래 길이: 가변 길이
- CNN 입력 길이: `100` time steps
- 방법: 잘린 trial 구간을 시간축 기준으로 보간해 `100` step으로 resample

즉, "trial을 어디서 어디까지 자를지"는 main pipeline과 같고, "잘린 trial을 CNN이 먹기 좋은 고정 길이 tensor로 다시 맞추는 단계"만 CNN 전용 추가 전처리라고 보면 된다.

## 왜 이 설명이 중요하나?

겉으로는 모두 `step vs. nonstep` 비교처럼 보여도, 전처리 정의가 달라지면 모델이 보는 문제가 달라진다.

예를 들어 아래 둘은 같은 질문이 아니다.

- 기존 helper로 자른 뒤 CNN에 넣는 경우
- 임의의 다른 window 규칙으로 다시 잘라 CNN에 넣는 경우

그래서 지금 CNN 결과를 synergy 분석과 비교하려면, 최소한 trial 선택과 분석 구간 정의는 같아야 한다.

이 문서에서 repeated하게 trial 수 sanity check를 강조하는 이유도 여기 있다.

- selected trials가 대략 `125`
- step trials가 `53`
- nonstep trials가 `72`
- subjects가 `24`

이 숫자가 크게 어긋나면, 모델 성능보다 먼저 전처리 정합성을 의심해야 한다.
...
## 현재 프로토타입에서는 무엇을 확인했나?

현재 `analysis/260312-0026-cnn_step_vs_nonstep/` 안에는 첫 실행 가능한 프로토타입이 들어 있다.
...
1. `configs/global_config.yaml`에 연결된 min-max normalized EMG와 event workbook을 읽는다.
2. 기존 저장소 helper를 사용해 기존과 같은 filtering logic을 적용하고, `platform_onset`부터 `analysis_window_end`까지 trial을 자른다.
3. step trial은 실제 `step_onset`, nonstep trial은 surrogate 또는 fallback 규칙으로 만들어진 `analysis_window_end`를 사용한다.
4. 선택된 가변 길이 trial을 `16 x 100` 고정 길이 tensor로 resample한다.
5. subject-wise split으로 baseline과 작은 1D CNN을 비교한다.

여기서 가장 중요한 sanity check는 trial 수다.

현재 기준으로는 아래 값이 나와야 filtering logic이 맞다고 본다.

- selected trials: `125`
- step trials: `53`
- nonstep trials: `72`
- subjects: `24`
- tensor shape: `(125, 16, 100)`

즉, 지금 단계에서는 "성능이 아주 높으냐"보다 먼저, 같은 filtering logic을 따라갔을 때 trial이 대략 `125개`가 맞게 잡히는지가 더 중요하다.

## 어떻게 실행하나?

먼저 dry-run으로 trial 수와 tensor shape를 확인한다.

```bash
conda run --no-capture-output -n cuda python analysis/260312-0026-cnn_step_vs_nonstep/analyze_cnn_step_nonstep.py --dry-run
```

그 다음 full run으로 baseline과 CNN을 비교한다.

```bash
conda run --no-capture-output -n cuda python analysis/260312-0026-cnn_step_vs_nonstep/analyze_cnn_step_nonstep.py
```

현재 full run의 평균 subject-wise 결과는 아래와 같다.

- Logistic regression: accuracy `0.720`, balanced accuracy `0.742`, F1 `0.689`, ROC AUC `0.847`
- Small 1D CNN: accuracy `0.752`, balanced accuracy `0.767`, F1 `0.712`, ROC AUC `0.863`

이 값은 "CNN이 이미 완성되었다"는 뜻은 아니다. 하지만 적어도 첫 프로토타입 기준에서는, 같은 trial 의미를 유지한 상태에서도 CNN이 baseline보다 조금 더 많은 정보를 잡아낼 가능성이 있다는 정도는 보여준다.
