...
def test_nonstep_surrogate_uses_absolute_mean_step_onset_even_if_platform_onset_varies(
    fixture_bundle: dict[str, Path],
    tmp_path: Path,
) -> None:
    """Nonstep window end should be the subject's mean absolute step_onset (README contract)."""
    cfg = load_pipeline_config(str(fixture_bundle["global_config"]))
    workbook_path = _write_event_workbook(
        tmp_path,
        [
            {
                "subject": "S01",
        ...
        "varying_platform_onset_events.xlsx",
    )
    prepared = load_event_metadata(str(workbook_path), cfg)
    nonstep = prepared.loc[prepared["step_TF"].astype(str).str.lower().eq("nonstep")].copy()
    assert nonstep.shape[0] == 2
    assert nonstep["analysis_window_source"].eq("subject_mean_step_onset").all()
    assert nonstep["analysis_window_is_surrogate"].astype(bool).all()
    assert np.allclose(nonstep["analysis_window_end"].astype(float), 12.0)
