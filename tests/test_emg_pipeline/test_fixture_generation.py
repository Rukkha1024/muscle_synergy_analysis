...
def test_synergy_fixture_config_explicitly_writes_gap_overrides(fixture_bundle: dict[str, object]) -> None:
    ...
    assert "gap_ref_n: 5" in config_text
    assert "gap_ref_restarts: 3" in config_text
    assert "uniqueness_candidate_restarts: 10" in config_text
    assert "disallow_within_trial_duplicate_assignment" not in config_text
