...
`run_manifest.json`은 현재 run의 실행 메타데이터를 기록한 manifest다. 현재 구현 기준으로 `created_at_utc`, `python_executable`, `python_version`, `config_sha256`, `runtime_seed`, `run_id`뿐 아니라 `selected_mode`, `analysis_modes`, root parquet 경로도 함께 저장한다. `analysis_methods_manifest.json`은 여기에 더해 mode별 output directory와 mode별 parquet alias 경로를 따로 정리한 manifest다.
