...
기본 선택 규칙은 event workbook에서 `mixed == 1`로 표시된 세트만 포함하는 것이다. 여기에 더해 현재 구현은 `young` 연령군만 선택하고, step trial은 dominant side와 일치하는 ipsilateral step만 남기며, nonstep trial은 같은 subject-velocity 세트에서 함께 해석한다. 선택된 step trial에 실제 `step_onset`이 없으면 해당 row는 제외되며, 남은 trial만 downstream 분석으로 전달된다.
...
이 규칙은 step과 non-step이 동일 피험자, 동일 comparison velocity 안에서 비교 가능한 세트로 정리되도록 강제한다. 따라서 본 저장소의 기본 산출물은 모든 원시 trial의 전수 요약이 아니라, 현재 prefilter를 통과한 curated mixed comparison set의 결과로 이해해야 한다.
...
4. [scripts/emg/04_cluster_synergies.py](scripts/emg/04_cluster_synergies.py)
   `analysis_selected_group == True`인 trial만 모아 mode별 하나의 `pooled_step_nonstep` 공간에서 clustering한다.
