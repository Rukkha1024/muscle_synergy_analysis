...
군집화는 [configs/synergy_stats_config.yaml](configs/synergy_stats_config.yaml)의 `synergy_clustering` 설정을 따른다. 현재 기본 grouping mode는 `global_step_nonstep`이며, 알고리즘은 `cuml_kmeans`, duplicate 제약은 `disallow_within_trial_duplicate_assignment: true`다. 즉 한 trial에서 추출된 복수 synergy가 동일 cluster에 중복 배정되지 않도록 제한한다.
