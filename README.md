...
`concatenated` mode에서는 `trial_num`이 실제 trial 번호 대신 `concat_step` 또는 `concat_nonstep`으로 기록된다. 이 경우 provenance는 `analysis_unit_id`, `source_trial_nums_csv`, `analysis_source_trial_count`를 함께 읽어야 한다. `analysis_unit_id`는 subject-level super-trial 식별자이고, `source_trial_nums_csv`는 그 분석 단위를 구성한 원래 trial 번호 목록이다.
