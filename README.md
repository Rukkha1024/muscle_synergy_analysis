...
<runtime.output_dir>/
  ...
  run_manifest.json
  global_step/
    clustering_metadata.csv
    cluster_labels.csv
    cluster_members.csv
    representative_W_posthoc.csv
    representative_H_posthoc_long.csv
    minimal_units_W.csv
    minimal_units_H_long.csv
    trial_window_metadata.csv
  global_nonstep/
    clustering_metadata.csv
    cluster_labels.csv
    cluster_members.csv
...
run 루트에 있는 `all_*.csv`는 `global_step`과 `global_nonstep` 결과를 합친 aggregate 산출물이고, 각 group 디렉터리 안의 CSV는 같은 스키마를 유지한 group-specific 산출물이다. figure는 `global_step_clusters.png`와 `global_nonstep_clusters.png` 두 장으로 저장되며, 각 cluster마다 왼쪽에 `W` bar plot, 오른쪽에 `H(100-window)` line plot을 배치한다. trial별 NMF 그림은 `figures/nmf_trials/` 아래에 `subject-velocity-trial-step_class`를 반영한 파일명으로 저장된다.
...
- `all_cluster_members.csv`
  `all_cluster_labels.csv`와 같은 membership 정보를 cluster 중심으로 읽기 쉽게 모은 표다.
  핵심 컬럼은 `group_id`, `subject`, `velocity`, `trial_num`, `trial_id`, `cluster_id`, `component_index`와 공통 `analysis_*` 메타데이터다.
  특정 `cluster_id`를 구성하는 trial/component 목록을 보고 싶을 때 이 파일이 가장 직접적이다.
...
- `global_step/*.csv`, `global_nonstep/*.csv`
  group 디렉터리 안의 `clustering_metadata.csv`, `cluster_labels.csv`, `cluster_members.csv`, `representative_W_posthoc.csv`, `representative_H_posthoc_long.csv`, `minimal_units_W.csv`, `minimal_units_H_long.csv`, `trial_window_metadata.csv`는 각각 run 루트의 `all_*` 파일과 같은 구조를 유지하되, 해당 group의 행만 담고 있다.
  따라서 step만 따로 읽고 싶으면 `global_step/`, nonstep만 따로 읽고 싶으면 `global_nonstep/`를 직접 보면 된다.
...
`results_interpretation.xlsx`는 run-level CSV를 사람 읽기 쉬운 Excel Table로 다시 정리한 workbook이다. `summary`, `trial_windows`, `cluster_labels`, `cluster_members`, `representative_W`, `representative_H`, `minimal_W`, `minimal_H`, `table_guide` 시트를 통해 CSV와 같은 내용을 설명형 문구와 함께 확인할 수 있다.
