...
기본 분석 구간은 `platform_onset ~ analysis_window_end`이며, step trial은 실제 `step_onset`을 종료점으로 사용한다. 반면 nonstep trial은 실제 step event가 존재하지 않으므로, 같은 피험자-velocity 안에서 선택된 step trial의 평균 step latency(`step_onset - platform_onset`)를 적용한 surrogate 종료점을 우선 사용한다. 다만 donor step이 없는 nonstep row는 코드상 `platform_offset`으로 fallback될 수 있으며, 이 여부는 `analysis_window_source`에 기록된다. 현재 최종 selected set은 기존 `mixed comparison` prefilter를 통과한 뒤 `(subject, velocity)` 기준으로 selected `step`과 `nonstep`이 모두 존재하는 paired key만 남긴 결과이며, 이 판단은 `analysis_selected_group_prepaired`, `analysis_pair_key`, `analysis_is_paired_key`, `analysis_pair_status`에 audit trail로 남는다.

## 1. 분석 목적과 비교 단위

본 파이프라인의 목적은 동일 피험자 내부에서 step 전략과 non-step 전략을 비교 가능한 시간 구조로 정렬한 뒤, 각 trial에서 추출된 근육 시너지를 공통 pooled cluster 공간에서 요약하는 데 있다. 이를 위해 분석의 기본 단위는 `subject-velocity-trial`로 두고, 비교 세트는 `mixed velocity` 조건과 현재 코드의 `replace_v3d_meta_prefilter` 규칙을 통과한 trial로 제한한다.

기본 선택 규칙은 event workbook에서 `mixed == 1`로 표시된 세트만 포함하는 것이다. 여기에 더해 현재 구현은 `young` 연령군만 선택하고, step trial은 dominant side와 일치하는 ipsilateral step만 남기며, nonstep trial은 같은 subject-velocity 세트에서 함께 해석한다. 선택된 step trial에 실제 `step_onset`이 없으면 해당 row는 제외되며, 마지막에는 `(subject, velocity)` 단위로 selected `step`과 `nonstep`이 모두 존재하는 paired key만 `analysis_selected_group == True`로 남는다.
...
분석 대상 trial은 [configs/emg_pipeline_config.yaml](configs/emg_pipeline_config.yaml)의 selection 설정과 [src/emg_pipeline/io.py](src/emg_pipeline/io.py)의 event prefilter 로직에 따라 선별된다. 현재 코드 기준 기본 규칙은 다음과 같다.

- `mixed_only: true`
- `mixed_column: "mixed"`
- `analysis_selection_rule: "replace_v3d_meta_prefilter"`
- `analysis_age_group == "young"`
- step trial은 dominant side와 일치하는 ipsilateral step만 선택
- nonstep trial은 같은 `subject-velocity` 안에서 함께 선택
- 선택된 step trial에 `step_onset`이 없으면 해당 row를 제외
- 마지막에 `(subject, velocity)` 단위 paired gate를 적용하여 selected `step`과 selected `nonstep`이 둘 다 있는 key만 최종 선택

이 규칙은 step과 non-step이 동일 피험자, 동일 comparison velocity 안에서 비교 가능한 세트로 정리되도록 강제한다. 따라서 본 저장소의 기본 산출물은 모든 원시 trial의 전수 요약이 아니라, 현재 prefilter와 paired gate를 모두 통과한 curated mixed comparison set의 결과로 이해해야 한다.
...
step trial의 경우 실제 `step_onset`이 `analysis_window_end`로 직접 사용된다. 반면 nonstep trial은 실제 step event가 존재하지 않으므로, 같은 피험자-velocity 안의 step trial 평균 step latency(`step_onset - platform_onset`)를 적용하여 `analysis_window_end = platform_onset + mean(step_onset - platform_onset)` 형태의 surrogate 종료점을 우선 정의한다. 다만 eligible donor step이 없는 nonstep row는 `platform_offset`으로 fallback될 수 있으며, 실제로 어떤 종료점이 사용됐는지는 `analysis_window_source`와 `analysis_window_is_surrogate`에 함께 기록된다. 이 규칙은 step과 non-step을 동일한 시간 기준에서 비교하기 위한 것이다.
...
4. [scripts/emg/04_cluster_synergies.py](scripts/emg/04_cluster_synergies.py)
   `analysis_selected_group == True`인 trial만 모아 mode별 하나의 `pooled_step_nonstep` 공간에서 clustering한다. 현재 이 컬럼은 raw prefilter 뒤의 paired gate까지 반영한 최종 selected set이다.
...
이 순서는 `자료 로딩 → trial 절단 → 시너지 추출 → pooled 군집화 → 산출물 저장`의 분석 절차와 대응한다.

현재 시너지 추출 mode는 [configs/synergy_stats_config.yaml](configs/synergy_stats_config.yaml)의 `synergy_analysis.mode` 또는 CLI `--mode {trialwise,concatenated,both}`로 선택한다. `trialwise`는 기존처럼 trial별로 NMF를 수행하는 방식이고, `concatenated`는 같은 `subject-velocity-step_class` 안의 trial을 먼저 이어 붙인 뒤 subject-level super-trial에 대해 NMF를 수행하는 방식이다. 여기서 `concatenated`는 모든 피험자를 하나의 global matrix로 합치는 뜻이 아니다. `both`를 선택하면 두 mode를 같은 run 안에서 독립적으로 실행하고, 결과도 서로 다른 하위 디렉터리에 분리 저장한다.
...
중요한 점은 대표 cluster를 한 개의 pooled 공간에서 학습하되, 각 component의 전략 라벨(`analysis_step_class`)은 그대로 유지한다는 것이다. 따라서 현재 main pipeline에서 같은 `cluster_id`는 step과 nonstep이 공유하는 pooled 대표 구조 번호로 읽어야 하며, 특정 cluster 안에 step과 nonstep이 어떻게 섞였는지는 `pooled_cluster_strategy_summary.csv`에서 확인한다.

또한 pooled clustering이라고 해서 trial identity가 사라지는 것은 아니다. 각 component에는 `subject`, `velocity`, `trial_num`, `group_id`, `analysis_step_class`가 함께 저장되며, duplicate feasibility도 여전히 같은 `(subject, velocity, trial_num)` 안에서만 검사한다. 즉 서로 다른 피험자 component를 한데 모아도, 최종 채택된 해에서는 한 trial에서 나온 여러 synergy가 같은 cluster에 겹쳐 들어가지 않는다. 대신 이 조건은 metadata의 `k_lb`, `k_gap_raw`, `k_selected`, `k_min_unique`, `selection_status`, `duplicate_trial_count_by_k_json`, `feasible_objective_by_k_json`에 함께 기록되어, 구조 기준 K와 feasibility 때문에 올린 K를 구분해 해석할 수 있다.

참고로 `global_step` / `global_nonstep` 명명은 cross-group similarity 유틸리티와 일부 하위 호환 테스트에 남아 있지만, 현재 `main.py` 기준 기본 산출물은 이 분리 그룹을 직접 export하지 않는다.
...
<runtime.output_dir>/
  ...
  all_concatenated_source_trial_windows.csv
...
- `group_id`: 현재 main pipeline에서는 `pooled_step_nonstep`
...
- `cluster_id`: pooled clustering 이후 배정된 대표 synergy 번호

`concatenated` mode에서는 `trial_num`이 실제 trial 번호 대신 `concat_step` 또는 `concat_nonstep`으로 기록된다. 이 경우 provenance는 두 층으로 나뉜다. `all_trial_window_metadata.csv`는 여전히 analysis unit 수준 요약 metadata를 보여주고, `analysis_unit_id`, `source_trial_nums_csv`, `analysis_source_trial_count`를 통해 어떤 source trial들이 묶였는지 빠르게 확인하는 데 쓴다. 반면 source trial별 analysis window provenance를 직접 보고 싶으면 `all_concatenated_source_trial_windows.csv`를 읽어야 한다. 이 새 파일에서 `trial_num=concat_step|concat_nonstep`는 부모 analysis unit이고, `source_trial_num`이 실제 원본 trial 번호다.
...
- `analysis_selected_group_prepaired`: paired gate를 적용하기 전 기존 prefilter 결과
- `analysis_selected_group`: 현재 mixed comparison selection에 포함되었는지 여부
- `analysis_pair_key`: `(subject, velocity)` 기준 paired audit key
- `analysis_is_paired_key`: 해당 key가 selected `step`과 selected `nonstep`을 모두 갖는지 여부
- `analysis_pair_status`: paired gate 판정 상태 문자열
- `analysis_selection_rule`: 어떤 selection rule을 통과했는지 기록한 문자열

`analysis_window_source`는 기본적으로 `actual_step_onset`, `subject_mean_step_onset`, `platform_offset` 중 하나를 가진다. 현재 기본 비교 구조에서는 step trial이 `actual_step_onset`, nonstep trial이 `subject_mean_step_onset`을 사용한다. paired audit 관점에서는 `analysis_selected_group_prepaired == True`이지만 `analysis_is_paired_key == False`인 row가 `step_only` 또는 `nonstep_only` key였음을 뜻한다.
...
- `final_summary.csv`
  run 수준에서 mode별 clustering 결과를 한 줄씩 요약한 summary다.
  ...
  현재 기본 `both` 실행에서는 보통 `aggregation_mode = trialwise | concatenated` 두 행이 생성되고, `group_id`는 둘 다 `pooled_step_nonstep`이다. 여기서 `n_trials`는 해당 mode에 포함된 analysis unit 수, `n_components`는 pooling된 전체 component 수, `n_clusters`는 최종 채택된 대표 synergy 개수다.

- `all_clustering_metadata.csv`
  group별 clustering 선택 과정을 기록한 run-level audit table이다.
  ...
  현재 기본 `both` 실행에서는 보통 `trialwise/pooled_step_nonstep`, `concatenated/pooled_step_nonstep` 두 행으로 읽으면 된다. 이 파일은 구조 기준 K와 duplicate-free feasibility 때문에 실제 선택된 K를 구분해서 읽을 때 사용한다.

- `all_trial_window_metadata.csv`
  trial 단위 메타데이터 long table이다. 각 행은 한 trial의 선택 결과와 window provenance를 나타낸다.
  핵심 컬럼은 `subject`, `velocity`, `trial_num`, `trial_id`, `status`, `n_components`, `vaf`, `extractor_type`, `extractor_backend`, `analysis_window_onset_column`, `analysis_window_offset_column`, `analysis_window_start`, `analysis_window_end`, `analysis_window_start_device`, `analysis_window_end_device`, `analysis_window_duration_device_frames`, `analysis_step_class`, `analysis_is_step`, `analysis_is_nonstep`, `analysis_window_source`, `analysis_window_is_surrogate`, `analysis_selected_group_prepaired`, `analysis_selected_group`, `analysis_pair_key`, `analysis_is_paired_key`, `analysis_pair_status`, `analysis_selection_rule`다.
  이 파일을 기준으로 step/nonstep 비교 세트와 trial 절단 구간을 해석한다. 다만 `concatenated` mode에서는 각 행이 source trial이 아니라 analysis unit 요약 row로 읽히므로, source-trial별 provenance를 확인할 때는 아래의 새 CSV를 함께 봐야 한다.

- `all_concatenated_source_trial_windows.csv`
  `concatenated` provenance 전용 long table이다. 한 행이 한 개의 source trial window를 뜻한다.
  핵심 컬럼은 `aggregation_mode`, `group_id`, `subject`, `velocity`, `trial_num`, `analysis_unit_id`, `source_trial_num`, `source_trial_order`, `source_step_class`, `analysis_window_source`, `analysis_window_start`, `analysis_window_end`, `analysis_window_length`, `analysis_window_is_surrogate`다.
  `concatenated` mode 또는 `both` 실행에서만 생성된다. `trial_num=concat_step|concat_nonstep`는 부모 analysis unit을, `source_trial_num`은 실제 원본 trial 번호를 뜻한다. 따라서 이 파일을 열면 어떤 `concat_step` 또는 `concat_nonstep`이 정확히 어떤 원본 trial window들로 구성되었는지 바로 읽을 수 있다.
...
- `pooled_cluster_strategy_summary.csv`
  pooled cluster 안에서 step과 nonstep이 어떻게 분포하는지 요약한 table이다.
  핵심 컬럼은 `group_id`, `cluster_id`, `strategy_label`, `n_rows`, `cluster_total_rows`, `fraction_within_cluster`다.
  같은 `cluster_id`가 step 편향인지, nonstep 편향인지, 혹은 두 전략이 함께 섞인 대표 구조인지를 가장 빠르게 확인할 때 사용한다.
...
`run_manifest.json`은 현재 run의 실행 메타데이터를 기록한 manifest다. 현재 구현 기준으로 `created_at_utc`, `python_executable`, `python_version`, `config_sha256`, `runtime_seed`, `run_id`뿐 아니라 `selected_mode`, `analysis_modes`, root parquet 경로도 함께 저장한다. `analysis_methods_manifest.json`은 여기에 더해 mode별 output directory, mode별 `final.parquet`, alias parquet 경로, workbook 경로, `concatenated` provenance CSV 경로를 따로 정리한 manifest다.

`outputs/final.parquet`는 원시 time-series EMG를 저장한 파일이 아니다. 현재 구현에서는 run-level `all_minimal_units_W.csv`와 같은 스키마를 parquet로 저장한 long-format artifact이며, `both` 실행 시에는 `trialwise`와 `concatenated`를 합친 combined parquet가 된다. 따라서 이 파일을 읽을 때는 `aggregation_mode`를 먼저 확인해야 한다. mode별 parquet를 직접 읽고 싶으면 `outputs/final_trialwise.parquet`와 `outputs/final_concatenated.parquet`를 사용한다.
...
mode를 하나만 실행하고 싶으면 `--mode trialwise` 또는 `--mode concatenated`를 사용하면 된다. 기본 설정을 그대로 둘 때는 [configs/synergy_stats_config.yaml](configs/synergy_stats_config.yaml)의 `synergy_analysis.mode: both`가 적용된다.
...
이 명령은 해당 run 디렉터리의 `figures/` 트리를 기본적으로 덮어쓴다. pooled 기본 워크플로에서는 `all_representative_W_posthoc.csv`, `all_representative_H_posthoc_long.csv`, `all_minimal_units_W.csv`, `all_minimal_units_H_long.csv`, `all_cluster_labels.csv`, `all_trial_window_metadata.csv`가 필요하며, 이 파일들 중 하나라도 없으면 즉시 실패한다. `cross_group_w_cluster_decision.csv` 같은 cross-group source는 `global_step/global_nonstep` 산출물이 실제로 존재할 때만 추가로 요구된다. curated non-figure 산출물은 다시 계산하지 않는다.

## 10. 현재 범위와 제한

- 기본 mixed selection은 `mixed == 1`을 전제로 하며, 현재 구현은 `replace_v3d_meta_prefilter` 규칙에 따라 `young` 연령군과 ipsilateral step을 우선 선택한다.
...
- prefilter 이후 선택된 row가 하나도 남지 않으면 파이프라인은 `ValueError`로 즉시 실패한다.
- 현재 main pipeline은 `pooled_step_nonstep`을 기본 group으로 사용하며, `global_step` / `global_nonstep`을 직접 export하는 cross-group 비교 워크플로는 기본 실행 경로에 포함되어 있지 않다.
