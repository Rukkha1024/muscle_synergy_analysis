...
군집화는 [configs/synergy_stats_config.yaml](configs/synergy_stats_config.yaml)의 `synergy_clustering` 설정을 따른다. 현재 grouping은 `global_step`과 `global_nonstep` 두 집단으로 고정되며, YAML에서 `synergy_clustering.grouping` 같은 legacy 키는 지원하지 않는다. 알고리즘은 `cuml_kmeans`, duplicate 제약은 `disallow_within_trial_duplicate_assignment: true`다. 즉 한 trial에서 추출된 복수 synergy가 동일 cluster에 중복 배정되지 않도록 제한한다.

군집화 단계에서 사용하는 k range는 아래 규칙으로 정의한다.

- `k range = [k_min, k_max]`
- `k_min`은 선택된 trial들에서 subject별 NMF component 수(`H` 구조 수)의 최대값 중 최댓값(`subject NMF Hmax`) 이상이어야 한다. (`k_min >= subject NMF Hmax`)
- `k_max`는 `synergy_clustering.max_clusters`와 전체 component 샘플 수를 고려하여 제한한다.

`disallow_within_trial_duplicate_assignment: true`를 사용하는 경우, KMeans의 초기 라벨 결과에서 같은 trial 내부 중복 배정이 발생하면 trial 내부 라벨 재할당을 통해 zero-duplicate 조건을 만족시키도록 처리한다. 이 재할당 규칙이 muscle synergy k-means clustering 방법론으로 적절한지 여부는 추후 별도 검토가 필요하다.
...
또한 global clustering이라고 해서 trial identity가 사라지는 것은 아니다. 각 component에는 `subject`, `velocity`, `trial_num`, `group_id`가 함께 저장되며, duplicate 제약도 여전히 같은 `(subject, velocity, trial_num)` 안에서만 검사한다. 즉 서로 다른 피험자 component를 한데 모아도, 한 trial에서 나온 여러 synergy가 같은 cluster에 겹쳐 들어가지는 않도록 막는다.
